...
/**
 * Saves and loads GridMap to/from a JSON file (spec §4.2: JSON-based
 * serialization).
 *
 * Format:
 * {
    ...
    private static GridMap fromJson(String json) {
        ...
        if (tilesBlock != null) {
            ...
            for (String colArr : colArrays) {
                if (colArr.isBlank() || !colArr.contains("]"))
                    continue;
                ...
                for (int r = 0; r < vals.length && r < rows; r++) {
                    ...
                    if (v.equals("1"))
                        map.setTile(new Vec2(c, r), TileType.WALL);
                }
                c++;
                if (c >= cols)
                    break;
