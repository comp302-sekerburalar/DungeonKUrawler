...
public class WaveEngine {
    ...
    public void startSession() { waveTickTimer.play(); startNextWave(); }

    private void pauseGameTimers()  { spawnTimer.pause(); aiTimer.pause(); knightTimer.pause(); }
    private void resumeGameTimers() { spawnTimer.play();  aiTimer.play();  knightTimer.play(); }

    public void pause()  { pauseGameTimers(); waveTickTimer.pause(); }
    public void resume() { if (state.isWaveActive()) resumeGameTimers(); waveTickTimer.play(); }
    public void stop()   { spawnTimer.stop(); aiTimer.stop(); knightTimer.stop(); waveTickTimer.stop(); }
    ...
    private void spawnNextEnemy() {
        ...
        if (state.getEnemiesSpawnedMut() >= state.getEnemiesThisWave()) {
            spawnTimer.pause(); return;
        }
        int maxOnField = Math.min(50, 5 + state.getCurrentWave());
        if (enemies.size() >= maxOnField) return;
    ...
    private Enemy buildScaledEnemy(String id, Enemy.Type type, Vec2 pos) {
        ...
        if (wave % 5 == 0) { hp += wave * 10; str += wave; }
        return new Enemy(id, type, pos, hp, str);
    }

    // ── AI tick ───────────────────────────────────────────────────────────────
    private void tickAI() {
        if (!state.isWaveActive() && enemies.isEmpty()) return;

        for (Enemy e : enemies) {
            e.tick(hero.getPos(), map);
            Vec2 proj = e.consumePendingProjectile();
            if (proj != null) projectiles.add(new Projectile(e.getPos(), proj, e));
        }

        List<Projectile> dead = new ArrayList<>();
        for (Projectile p : projectiles) {
            if (!p.isActive()) { dead.add(p); continue; }
            p.advance(map);
            if (!p.isActive()) { dead.add(p); continue; }
            if (p.getPos().equals(hero.getPos())) {
                ...
                p.destroy(); dead.add(p);
                notifyStats(); checkDeath();
            }
        }
    ...
    public boolean moveHero(int dc, int dr) {
        if (state.isSessionOver()) return false;
        Vec2 target = hero.getPos().add(dc, dr);
        if (!map.isPassable(target)) { post("Blocked!"); return false; }
        hero.setPos(target);
        hero.spendEnergy(Hero.ENERGY_COST_WALK);
        notifyStats(); notifyMap();
        return true;
    }

    // ── Hero attacks enemy ────────────────────────────────────────────────────
    public void heroAttackEnemy(Enemy enemy) {
        if (!map.isAdjacent(hero.getPos(), enemy.getPos())) { post("Too far!"); return; }
        if (!hero.hasWeaponEquipped()) { post("No weapon equipped!"); return; }
        int dmg = CombatSystem.heroAttack(hero, enemy);
        post("Hit " + enemy.getType() + " for " + dmg + "!");
        if (!enemy.isAlive()) onEnemyKilled(enemy);
        notifyStats(); notifyMap();
    }
    ...
    private void onEnemyKilled(Enemy enemy) {
        ...
        if (state.getCurrentStreak() > 1) msg += " (x" + state.getCurrentStreak() + " streak!)";
        if (coins > 0) msg += "  \uD83E\uDE99+" + coins;
        post(msg);

        if (coins > 0 && onCoinDrop != null) onCoinDrop.accept(coins);
        notifyState();
    ...
    private void waveComplete() {
        ...
        post("=== WAVE " + state.getCurrentWave() + " COMPLETE! ===  Shop opens for " + (int)diff.marketBreakSeconds + "s");
        state.openMarket(diff.marketBreakSeconds);
        notifyState();
        if (onWaveComplete != null) onWaveComplete.run();
    }

    // ── Timer ticks ───────────────────────────────────────────────────────────
    private void tickTimers() {
        if (state.isWaveActive()) state.tickWaveTimer(0.1);
        if (state.isMarketOpen()) {
            state.tickMarketTimer(0.1);
            if (state.getMarketTimerSec() <= 0) {
                state.closeMarket();
                if (onMarketClose != null) onMarketClose.run();
                startNextWave();
            }
        }
        notifyState();
    ...
    public boolean useConsumable(int slot) {
        ...
        if (slot < 1 || slot > keys.size()) return false;

        String id = keys.get(slot - 1);
        if (!userStore.useConsumable(heroName, id)) return false;
    ...
    // ── Marketplace ───────────────────────────────────────────────────────────
    public List<MarketItem> getShopInventory() { return Collections.unmodifiableList(shopInventory); }
    public boolean canAfford(MarketItem item)  { return userStore.getCoins(hero.getName()) >= item.getPrice(); }
    ...
    public boolean purchase(MarketItem item) {
        ...
        if (item.getCategory() == MarketItem.Category.POWER_UP) applyPermanentEffect(item);

        purchasedIds.add(item.getId());
        post("Purchased: " + item.getName());
        notifyStats(); notifyState();
        if (onInventoryChanged != null) onInventoryChanged.run();
        return true;
    }
    ...
    private void applyLoadout(String heroName) {
        ...
        if (starterWeapon == null) starterWeapon = GameObjects.dagger(hero.getPos());
    ...
    private void applyArmorBonus(String armor) {
        ...
        if (def > 0) hero.modifyStat(StatType.DEF, def);
    }

    // ── Death check ───────────────────────────────────────────────────────────
    private void checkDeath() {
        if (!hero.isAlive()) {
            stop(); state.endSession();
    ...
    // ── Notifications ─────────────────────────────────────────────────────────
    private void notifyMap()   { if (onMapChanged   != null) onMapChanged.run(); }
    private void notifyStats() { if (onStatsChanged != null) onStatsChanged.run(); }
    private void notifyState() { if (onStateChanged != null) onStateChanged.run(); }
    private void post(String m){ System.out.println("[Wave] " + m); if (onMessage != null) onMessage.accept(m); }
    ...
    public void setOnCoinDrop(Consumer<Integer> c)  { onCoinDrop         = c; }
    public void setOnWaveComplete(Runnable r)       { onWaveComplete     = r; }
    public void setOnMarketClose(Runnable r)        { onMarketClose      = r; }
    public void setOnGameOver(Runnable r)           { onGameOver         = r; }
    public void setOnInventoryChanged(Runnable r)   { onInventoryChanged = r; }

    // ── Accessors ─────────────────────────────────────────────────────────────
    public GridMap          getMap()         { return map; }
    public Hero             getHero()        { return hero; }
    public WaveState        getState()       { return state; }
    public WaveDifficulty   getDiff()        { return diff; }
    ...
    public int              getSkinIndex()   { return heroSkinIndex; }
    public UserStore        getUserStore()   { return userStore; }
    public boolean          isPurchased(String id) { return purchasedIds.contains(id) || userStore.ownsItem(hero.getName(), id); }
}
