...
public class GameScreen extends BaseScreen {
    ...
    public GameScreen(ScreenManager manager, String heroName,
        ...
        this(manager, heroName, existingMap, GameEngine.Mode.PLAY);
    }

    /** Play with a pre-designed map and explicit game mode. */
    public GameScreen(ScreenManager manager, String heroName,
            com.kurawler.engine.GridMap existingMap, GameEngine.Mode mode) {
        super(manager, true);
        this.heroName = heroName;
        this.engine = new GameEngine(heroName, existingMap, mode);
        initView();
        wireEngine();
        engine.start();
    ...
    protected Pane buildUI() {
        ...
        mapCanvas.widthProperty().bind(canvasStack.widthProperty());
        mapCanvas.heightProperty().bind(canvasStack.heightProperty());
        mapCanvas.widthProperty().addListener((obs, oldV, newV) -> drawMap());
        mapCanvas.heightProperty().addListener((obs, oldV, newV) -> drawMap());
    ...
    private HBox buildHUD() {
        ...
        lblTargetRelic = makeLabel(engine.isTeamMatch()
                ? "TEAM MATCH: BLUE vs RED"
                : "FIND: " + engine.getTargetRelicName(), "#7ee8fa", 10, true);
    ...
    private void drawMap() {
        ...
        double tile = currentTile();
        double ox = mapOffsetX(tile);
        double oy = mapOffsetY(tile);
        ...
        for (int c = 0; c < grid.getCols(); c++) {
            for (int r = 0; r < grid.getRows(); r++) {
                ...
                double px = ox + c * tile, py = oy + r * tile;
        ...
        for (int dc = -1; dc <= 1; dc++) {
            for (int dr = -1; dr <= 1; dr++) {
                ...
                if (cell.inBounds(grid.getCols(), grid.getRows()))
                    gc.fillRect(ox + cell.col() * tile, oy + cell.row() * tile, tile - 1, tile - 1);
            }
        }
        ...
        for (Projectile p : engine.getProjectiles()) {
            if (!p.isActive())
                continue;
            GameRenderer.drawProjectile(gc, ox + p.getPos().col() * tile, oy + p.getPos().row() * tile, tile);
        }

        // Enemies
        for (Enemy e : engine.getEnemies())
            drawEnemy(gc, e, tile, ox, oy);

        if (engine.hasShadowClone())
            drawShadowClone(gc, engine.getShadowClonePos(), tile, ox, oy);

        // Hero
        drawHero(gc, heroPos, tile, ox, oy);
    }

    private void drawObject(GraphicsContext gc, GameObject obj, double tile, double ox, double oy) {
        double px = ox + obj.getPos().col() * tile;
        double py = oy + obj.getPos().row() * tile;
    ...
    private void drawHero(GraphicsContext gc, Vec2 pos, double tile, double ox, double oy) {
        double px = ox + pos.col() * tile, py = oy + pos.row() * tile;
    ...
    private void drawShadowClone(GraphicsContext gc, Vec2 pos, double tile, double ox, double oy) {
        double px = ox + pos.col() * tile, py = oy + pos.row() * tile;
        gc.setGlobalAlpha(0.55);
        heroAnimator.draw(gc, px, py, tile, tile);
        gc.setGlobalAlpha(1.0);
        gc.setStroke(Color.web("#7ee8fa"));
        gc.setLineWidth(2);
        gc.strokeRect(px + 3, py + 3, tile - 7, tile - 7);
    }

    private void drawEnemy(GraphicsContext gc, Enemy enemy, double tile, double ox, double oy) {
        double px = ox + enemy.getPos().col() * tile;
        double py = oy + enemy.getPos().row() * tile;
    ...
    private double currentTile() {
        double w = mapCanvas == null ? TILE * MAP_W : mapCanvas.getWidth();
        double h = mapCanvas == null ? TILE * MAP_H : mapCanvas.getHeight();
        return Math.max(1, Math.min(w / MAP_W, h / MAP_H));
    }

    private double mapOffsetX(double tile) {
        return Math.max(0, (mapCanvas.getWidth() - tile * MAP_W) / 2.0);
    }

    private double mapOffsetY(double tile) {
        return Math.max(0, (mapCanvas.getHeight() - tile * MAP_H) / 2.0);
    }

    private Vec2 cellFromCanvas(double mouseX, double mouseY) {
        double tile = currentTile();
        int col = (int) Math.floor((mouseX - mapOffsetX(tile)) / tile);
        int row = (int) Math.floor((mouseY - mapOffsetY(tile)) / tile);
    ...
    private void handleMapClick(double mouseX, double mouseY) {
        Vec2 clicked = cellFromCanvas(mouseX, mouseY);
        if (!clicked.inBounds(engine.getMap().getCols(), engine.getMap().getRows()))
            return;
    ...
    private void showEnemyActions(Enemy enemy) {
        ...
        if (canAttackEnemy(enemy)) {
            ...
            String msg = !isAttackableEnemy(enemy)
                    ? "Blue team ally."
                    : engine.getHero().hasWeaponEquipped()
    ...
    private boolean isAttackableEnemy(Enemy enemy) {
        return !engine.isTeamMatch() || enemy.getTeam() == 1;
    }

    private boolean canAttackEnemy(Enemy enemy) {
        return isAttackableEnemy(enemy)
                && engine.getHero().hasWeaponEquipped()
    ...
    private void attackSelectedEnemy() {
        if (selectedEnemy == null)
            return;
        if (!canAttackEnemy(selectedEnemy)) {
            showEnemyActions(selectedEnemy);
            drawMap();
            return;
        }
        engine.heroAttackEnemy(selectedEnemy);
        selectedEnemy = null;
        actionPanel.getChildren().clear();
        actionPanel.getChildren().add(grayLabel("Click enemy to attack"));
        drawMap();
    }
    ...
    private void handleKey(KeyCode code) {
        ...
        switch (code) {
            ...
            case SPACE -> attackSelectedEnemy();
            case ESCAPE -> {
                engine.pause();
                showInGameMenu();
            }
    ...
    private void showVictoryScreen() {
        ...
        a.setTitle("Victory!");
        a.setHeaderText(engine.isTeamMatch()
                ? "TEAM MATCH COMPLETE"
                : "YOU FOUND THE " + engine.getTargetRelicName().toUpperCase() + "!");
        a.setContentText(engine.isTeamMatch()
                ? engine.getVictoryMessage()
                : "The dungeon yields its secret. You are victorious!");
        ButtonType menu = new ButtonType("Main Menu");
