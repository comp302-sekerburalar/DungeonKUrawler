...
public class WaveState {
    ...
    public void tickWaveTimer(double dtSec)    { waveTimerSec   += dtSec; }
    public void tickMarketTimer(double dtSec)  { marketTimerSec -= dtSec; }
    ...
    public int recordKill(int baseScore, WaveDifficulty diff) {
        ...
        double scoreMult = currentStreak >= diff.streakBonusThreshold
            ? diff.streakScoreMultiplier : 1.0;
        totalScore += (long)(baseScore * scoreMult);

        // Coin drop
        double coinProb = diff.coinDropChance;
        if (currentStreak >= diff.streakBonusThreshold) coinProb *= diff.streakCoinMultiplier;
        int dropped = 0;
    ...
    public void breakStreak() { currentStreak = 0; }
    ...
    public void endSession() { sessionOver = true; }
    ...
    public int     getEnemiesLeft()      { return Math.max(0, enemiesThisWave - enemiesKilled); }
    public boolean isWaveActive()        { return waveActive; }
    public boolean isMarketOpen()        { return marketOpen; }
    public double  getWaveTimerSec()     { return waveTimerSec; }
    public double  getMarketTimerSec()   { return marketTimerSec; }
    ...
    int  getEnemiesSpawnedMut()           { return enemiesSpawned; }
    void incrementSpawned()               { enemiesSpawned++; }
    void setWaveActive(boolean b)         { waveActive = b; }
}
