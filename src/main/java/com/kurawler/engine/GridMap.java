...
public class GridMap {
    ...
    // ── tile ──
    public void     setTile(Vec2 p, TileType t) { baseTiles[p.col()][p.row()] = t; }
    public TileType getTile(Vec2 p)             { return baseTiles[p.col()][p.row()]; }
    ...
    public void removeObject(GameObject obj) {
        ...
        if (list != null) { list.remove(obj); if (list.isEmpty()) objects.remove(obj.getPos()); }
    }
    ...
    public boolean isPassable(Vec2 pos) {
        ...
        for (GameObject obj : objectsAt(pos)) if (obj.blocksMovement()) return false;
        return true;
    }
    ...
    public List<Vec2> neighbours4(Vec2 pos) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (n.inBounds(cols, rows)) r.add(n);
        }
        return r;
    }
    ...
    public void buildBorderWalls() {
        for (int c = 0; c < cols; c++) { baseTiles[c][0] = TileType.WALL; baseTiles[c][rows-1] = TileType.WALL; }
        for (int r = 0; r < rows; r++) { baseTiles[0][r] = TileType.WALL; baseTiles[cols-1][r] = TileType.WALL; }
    }
        ...
        for (int c = 1; c < cols-1; c++) {
            for (int r = 1; r < rows-1; r++) {
                ...
                for (Vec2 n : neighbours4(new Vec2(c,r)))
                    if (baseTiles[n.col()][n.row()] == TileType.WALL) { adj = true; break; }
                if (adj) result.add(new Vec2(c,r));
            }
        }
        return result;
    }
    ...
    public List<Vec2> allFloorCells() {
        ...
        for (int c = 1; c < cols-1; c++)
            for (int r = 1; r < rows-1; r++)
                if (isPassable(new Vec2(c,r))) result.add(new Vec2(c,r));
        return result;
    }
}
