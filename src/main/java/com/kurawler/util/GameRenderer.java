...
public final class GameRenderer {
    ...
    public static void drawObject(GraphicsContext gc, GameObject obj,
        ...
        // Special draw cases
        if ("SEARCH_WALL".equals(tag)) { drawSearchableWall(gc, px, py, tile); return; }
        if ("WEAPON".equals(tag))      { ImageCache.draw(gc, weaponFile(obj), px+tile*0.1, py+tile*0.1, tile*0.8, tile*0.8); return; }
        if ("ARMOR".equals(tag))       { drawArmor(gc, px, py, tile); return; }
        if ("RELIC".equals(tag))       { drawRelic(gc, px, py, tile); return; }
    ...
    public static void drawEnemy(GraphicsContext gc, Enemy enemy,
        ...
        if (enemy.getType() == Enemy.Type.KNIGHT) {
            if (chasing) { gc.setFill(Color.web("#e74c3c", 0.20)); gc.fillRect(px, py, tile, tile); }
            knightAnim.draw(gc, px, py, tile, tile);
        } else {
            // Sorcerer — animated frames
            if (chasing) { gc.setFill(Color.web("#9b59b6", 0.25)); gc.fillRect(px, py, tile, tile); }
    ...
    public static void drawProjectile(GraphicsContext gc, double px, double py, double tile) {
        ...
        gc.setFill(Color.web("#7ee8fa")); gc.fillOval(cx, cy, 10, 10);
        gc.setStroke(Color.web("#ffffff", 0.6)); gc.setLineWidth(1); gc.strokeOval(cx, cy, 10, 10);
    }
